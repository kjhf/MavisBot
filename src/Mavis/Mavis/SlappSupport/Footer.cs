...
namespace Mavis.SlappSupport
{
  public static class Footer
  {
    ...
    public static readonly IReadOnlyList<string> Footers = new[]
    {
      ...
      "Alright so we're checking out the only place that does Splatoon verifications coming at you from a sentient AI... It's " + Constants.ProgramName + ". ",
      "More Splatoon content, goddammit. ",
      "Hello it is " + Constants.ProgramName + " bringing you yet another Slapp result. ",
      "Not sponsored by any mobile game or VPN software. ",
      "OH FECK! ",
      "...and then I spoke. ",
    };

    public static string GetRandomFooterPhrase()
    {
      return random.Choice(Footers)?.Replace("%weapon%", Weapon.GetRandomWeapon()) ?? "<Unexpected Footer>";
    }
  }
}